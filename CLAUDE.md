...
注意**只有请求方向加密**：`securityData` 是密文，但响应不加密——`result`
直接就是明文 JSON 对象（`token`、`userName`、`virtualId`、`grpPlanCode` 等字段），直接读
`result.token` 即可，不要去解密。
...
登录成功返回一个 JWT。每次运行都重新登录、不做任何本地缓存——拿到 token 后 `setToken`
进内存供后续请求使用，进程结束即丢弃。（早先版本会把 token 缓存到 `.token` 文件并用
`getPersonalInfo` 校验有效性，现已移除，图简单。）
...
- EOB（理赔说明书）是**惰性生成**的，不在任何 JSON 字段里：直接 GET 那张静态图基本都 404，必须先 POST
  `/appwechat/ob/getEobFileInfo`（body `{ claimNo, flag: 'app' }`）触发服务端生成。它返回
  `result.picUrl`（图片）和
  `result.url`（同名 PDF）两个 URL，两者都下载（页面显示图片、点击打开 PDF）。`success !== 't'` 或无
  `picUrl` 表示这条没有 EOB。注意 EOB 与理赔是否结案**无关**，处理中的理赔也可能有。
...
- `src/api/`
  —— 每个接口一个模块：`persons`、`policies`、`claim-list`、`claim`（详情）、`eob`（理赔说明书）。
